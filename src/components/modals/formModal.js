...
export async function createEditWorkshopModal({ data = {}, onSubmit }) {
  ...
  placeLabel.textContent = "Lugar";
    ...
    placeInput.value = data.place || ""; // Usar el valor existente si hay datos
   ...
   if (data.coordinates) {
     if (typeof data.coordinates === 'string') {
       ...
       try {
         ...
         if (lat && lng) {
           coordinatesValue = JSON.stringify({ lat: parseFloat(lat), lng: parseFloat(lng) });
         }
       }
     } else if (typeof data.coordinates === 'object') {
       // Si ya es un objeto, convertirlo al formato correcto
       coordinatesValue = JSON.stringify({ lat: data.coordinates.lat, lng: data.coordinates.lng });
     }
   }
  ...
  geocodeStatus.style.fontSize = "12px";
    ...
    const addressExamples = [
      ...
      { place: "Lugar del Workshop", address: "Avenida de San Sebastián, 51, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Avenida Constitución, 1, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle del Castillo, 1, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle Consistorio, 9, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle José Murphy, 12, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Plaza de España, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Plaza del Príncipe, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Plaza de la Candelaria, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle Molinos de Agua, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Calle de la Concepción, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Calle Carrera, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Plaza del Adelantado, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Calle Vía Láctea, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Avenida de Anaga, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Avenida Francisco La Roche, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle San Simón, El Sauzal, España" },
      { place: "Lugar del Workshop", address: "Calle San Francisco, La Orotava, España" },
      { place: "Lugar del Workshop", address: "Calle Tomás Zerolo, La Orotava, España" },
      { place: "Lugar del Workshop", address: "Calle Retama, Puerto de la Cruz, España" },
      { place: "Lugar del Workshop", address: "Camino Cruz de Leandro, El Sauzal, España" },
      { place: "Lugar del Workshop", address: "Avenida de Los Cristianos, Arona, España" },
      { place: "Lugar del Workshop", address: "Avenida Loro Parque, Puerto de la Cruz, España" },
      { place: "Lugar del Workshop", address: "Avenida Siam, Costa Adeje, España" },
      { place: "Lugar del Workshop", address: "Carretera TF-21, La Orotava, España" },
      { place: "Lugar del Workshop", address: "Carretera del Norte, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Carretera General del Sur, Granadilla de Abona, España" },
      { place: "Lugar del Workshop", address: "Muelle de Anaga, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Muelle Pesquero, Puerto de la Cruz, España" },
      { place: "Lugar del Workshop", address: "Plaza de la Patrona de Canarias, Candelaria, España" },
      { place: "Lugar del Workshop", address: "Plaza Isla de la Madera, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Calle General, Santa Cruz de Tenerife, España" },
      { place: "Lugar del Workshop", address: "Avenida Principal, La Laguna, España" },
      { place: "Lugar del Workshop", address: "Calle Secundaria, Puerto de la Cruz, España" },
      { place: "Lugar del Workshop", address: "Avenida Central, La Orotava, España" },
      { place: "Lugar del Workshop", address: "Calle Mayor, El Sauzal, España" },
      { place: "Lugar del Workshop", address: "Avenida del Mar, Costa Adeje, España" },
      { place: "Lugar del Workshop", address: "Calle Real, Arona, España" },
      { place: "Lugar del Workshop", address: "Avenida de la Paz, Granadilla de Abona, España" },
    ...
    function showSuggestions(input, isPlaceInput = false) {
      ...
      filteredExamples.slice(0, 6).forEach(example => {
        ...
        suggestionDiv.addEventListener("click", () => {
          ...
          showToast("Dirección copiada. Haz clic en 'Obtener coordenadas'", "info");
        });
  ...
  async function geocodeAddress() {
    ...
    const place = placeInput.value.trim();
     ...
     if (address && address.length < 3) {
       geocodeStatus.textContent = "❌ La dirección es demasiado corta. Añade más detalles";
       geocodeStatus.style.color = "#dc2626";
    ...
    geocodeStatus.style.color = "#2563eb";
       ...
       let response = await fetch(
         `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAddress)}&limit=10&addressdetails=1&countrycodes=es`
       );
       ...
       console.log("📡 Respuesta de Nominatim:", data);
         ...
         if (!data || data.length === 0) {
           console.log("🔍 No se encontraron resultados, intentando con 'Tenerife'");
           response = await fetch(
             `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAddress + ", Tenerife")}&limit=10&addressdetails=1&countrycodes=es`
           );
          ...
          if (!data || data.length === 0) {
            ...
            response = await fetch(
              `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(address + ", Tenerife")}&limit=10&addressdetails=1&countrycodes=es`
            );
       ...
       if (data && data.length > 0) {
         ...
         const displayName = result.display_name || "";
          ...
          coordinatesInput.value = JSON.stringify({ lat: lat, lng: lon });
                   ...
                   if (importance > 0.02) {
            geocodeStatus.textContent = `✅ Coordenadas obtenidas: ${lat.toFixed(6)}, ${lon.toFixed(6)}`;
            geocodeStatus.style.color = "#059669";
            showToast("Coordenadas obtenidas correctamente", "success");
          } else {
            geocodeStatus.textContent = `⚠️ Coordenadas obtenidas (baja precisión): ${lat.toFixed(6)}, ${lon.toFixed(6)}`;
            geocodeStatus.style.color = "#f59e0b";
            showToast("Coordenadas obtenidas con baja precisión. Considera ser más específico", "warning");
          }
       } else {
         geocodeStatus.textContent = "❌ No se encontraron coordenadas para esta dirección";
         geocodeStatus.style.color = "#dc2626";
         showToast("No se encontraron coordenadas para esta dirección", "error");
         
                              // Sugerencias específicas para mejorar la búsqueda
           setTimeout(() => {
             geocodeStatus.textContent = "💡 Sugerencias: Usa una de las direcciones de los ejemplos o añade 'España' al final";
             geocodeStatus.style.color = "#2563eb";
           }, 3000);
       }
    } catch (error) {
      console.error("Error en geocodificación:", error);
      geocodeStatus.textContent = "❌ Error al obtener coordenadas. Verifica tu conexión";
      geocodeStatus.style.color = "#dc2626";
  ...
  function validateAddressFields() {
    ...
    const place = placeInput.value.trim();
       ...
       geocodeStatus.textContent = "⚠️ La dirección es muy corta. Añade más detalles";
       geocodeStatus.style.color = "#f59e0b";
     } else if (address.length >= 5) {
       geocodeStatus.textContent = "✅ Dirección válida. Puedes obtener coordenadas";
       geocodeStatus.style.color = "#059669";
     } else {
